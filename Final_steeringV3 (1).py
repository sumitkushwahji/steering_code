...
def read_current_Rb():
    ...
    if received_checksum != calculated_checksum_int:
        print(
            f"Checksum mismatch: received {received_checksum}, calculated {calculated_checksum_int}"
        )
        check_sum_match = 0
...
def send_cmd_Rb(apply_corr, lock_flag):
    ...
    if lock_flag == 0:  # PID is not in lock mode
        ...
        curr_value, hex_before, bytes_before, matching_check_sum = (
            read_current_Rb()
        )  # Get the current frequency offset value from the Rb
    ...
    shift_in_device_units_clamped = max(
        min(shift_in_device_units, max_int32), min_int32
    )
    ...
    cmnd = (
        "2e 09 00 27 "
        + " ".join(shift_hex[i : i + 2] for i in range(0, len(shift_hex), 2))
        + compute_checksum(shift_hex).replace("0x", "")
    )
    ...
    if lock_flag == 1:
        curr_new_value, hex_after, bytes_after, matching_check_sum = (
            read_current_Rb()
        )  # Get the current frequency offset value from the Rb
        Correction_info = {
            "Time stamp": datetime.now(),
            "Before_correction": curr_value,
            "After_correction": curr_new_value,
            ...
            "Check_sum_match": matching_check_sum,
        }
...
def is_it_today():
    ...
    return current_local_time.hour > 5 or (
        current_local_time.hour == 5 and current_local_time.minute >= 30
    )
...
def latest_file_in_directory(directory, file_prefix):
    ...
    if not files:
        ...
        readable_time = datetime.fromtimestamp(modification_time).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
...
def read_file_data(file_path):
    ...
    with open(file_path, "r") as file:
        ...
        df_split = df_split[
            df_split["SAT"].notna()
        ]  # Skip the lines where SAT column is missing
        ...
        def convert_sttime_to_seconds(sttime_str):
            ...
            hours, minutes, seconds = map(
                int, [sttime_str[:2], sttime_str[2:4], sttime_str[4:6]]
            )
            return (hours * 3600 + minutes * 60 + seconds) / 86400

        # Apply the conversion to STTIME and add it to MJD
        df_split["MJD"] += df_split["STTIME"].apply(
            lambda x: convert_sttime_to_seconds(x)
        )
    ...
    unique_mjd_int_values1 = sorted(
        set(int(mjd) for mjd in unique_mjd_values if not pd.isna(mjd))
    )
...
def process_CV(df1, df2, unique_MJD_times, unique_SATs, Freq_used):
    ...
    df1_filtered = df1[
        df1["MJD"].isin(unique_MJD_times)
        & df1["SAT"].isin(unique_SATs)
        & df1["FRC"].isin(Freq_used)
    ]
    df2_filtered = df2[
        df2["MJD"].isin(unique_MJD_times)
        & df2["SAT"].isin(unique_SATs)
        & df2["FRC"].isin(Freq_used)
    ]

    # Merge the filtered DataFrames
    merged_df = pd.merge(
        df1_filtered, df2_filtered, on=["SAT", "MJD", "FRC"], suffixes=("_df1", "_df2")
    )
...
def apply_CV_corrections(
    ...
    delta_f2 = (
        (0 - Present_error) / (5 * 16 * 60)
    ) * 1e-2  # Phase time constant 24 minutes = 24*60 sec
    ...
    new_row = {
        ...
        "CV_Time_Diff": Present_error,
        "delta_f1": delta_f1,
        "delta_f2": delta_f2,
        "delta_f3": delta_f3,
        "Corr_applied": total_correction,
    }
...
while True:
    ...
    # When the receiver is in Time transfer Mode
        ...
        first_session = (
            True  # Flag to consider the available files as first session f iles
        )
        ...
        rept_search_time = (
            60  # Waiting time to check for the new CGGTTS files  (seconds)
        )
        ...
        while True:
            ...
            while not (
                DO_file_found and Ref_file_found
                ...
                if os.path.exists(DO_folder_path) and not Got_New_DO_file:
                    ...
                    print(
                        "Waiting for the new MJD folder to be created for DO files..."
                    )
                ...
                if os.path.exists(Ref_folder_path) and not Got_New_Ref_file:
                    ...
                    if result is not None:
                        ...
                        if Ref_latest_file is not None and (
                            ...
                            Reffile_time_UTC = datetime.utcfromtimestamp(
                                time_of_fileRef
                            )
                            # Get the current UTC time
                            current_utc_time = datetime.utcnow()
                            ...
                            print(
                                f"No new Ref file available after : {Ref_latest_file}"
                            )
                            Ref_file_found = False
                    else:
            ...
            if first_session and both_files_found:
                ...
                print(
                    f"REF Latest file 1st session {Ref_latest_file} read successfully"
                )
                ...
                if DO_latest_file is not None and Previous_DO_file != DO_latest_file:
                    ...
                    df_DO, Un_time_DO, Un_SAT_DO, Un_freq_DO = read_file_data(
                        file_path_DO
                    )
                ...
                if Ref_latest_file is not None and Previous_Ref_file != Ref_latest_file:
                    ...
                    Un_time_Ref_str = ", ".join(
                        str(time_val) for time_val in Un_time_Ref
                    )
            ...
            if not df_DO.empty and not df_Ref.empty and both_files_found:
                ...
                if sorted(set(df_DO["MJD"]).intersection(set(df_Ref["MJD"]))) != 0:
                    ...
                    all_unique_MJDs = sorted(
                        set(df_DO["MJD"]).union(set(df_Ref["MJD"]))
                    )
                    Freq_CV = (
                        "LSC"  # available frequencies are LSC, L5C, 5S, 1S for IRNSS
                    )
                    ...
                    if set(df_DO["MJD"]) == (
                        ...
                        if (
                            ...
                            if (CV_session > 1) and not first_session:
                                ...
                                half_trkl = seconds_to_add = (
                                    df_DO["TRKL"].unique()[0] / 2
                                )
                                ...
                                if (
                                    ...
                                    first_session = False
                                    DO_file_found = (
                                        False  # Repeat search for the new files
                                    )
                                    Ref_file_found = False
                                    both_files_found = False
                                else:
                                ...
                                print(
                                    f"Time Lapsed since NaVIC mode: {time_lapsed_CVmode}"
                                )
                                correction_delay = 0
                                apply_CV_corrections(
                                    CV_session,
                    ...
                    print(
                        "These files doesnt belong to the same timeperiod to perform CV "
                    )
                    # first_session = False
                    DO_file_found = False  # Repeat search for the new files
                    Ref_file_found = False
            ...
            time.sleep(
                rept_search_time
            )  # Waiting time to check for the new CGGTTS files
