...
# def twos_complement(value_bin):
#     if value_bin >= 2**32:
#         # If the input value is out of range
#         print("Input number is out of range to perform the twos compliment ", value_bin )
#         return 0
#     else:
#         #print("with out TWOS compliment:", value_bin)
#         twos_comp_res= value_bin- int((value_bin<<1) & 2**32)
#         #print("TWOS compliment:", twos_comp_res)
#         return twos_comp_res
...
# Method to read the current Rb frequency offset from the Rb

# def read_current_Rb():   
#     Read_RAM = "2D 04 00 29"
#     #force_close_com_port('COM12')
#     print("Reading the current shift from Rb")

#     try:
#         data_write = Rb_ser.write(bytes.fromhex(Read_RAM))
...
#     hex_str = "".join("{:02x}".format(c) for c in data_read if c != 0x00 and c != 0x2d)
#     #hex_str = "".join("{:02x}".format(c) for c in data_read if c != 0x2d)
#     curr_shift = " ".join(hex_str[i:i+2] for i in range(4, len(hex_str)-2, 2))

#     hex_bytes = bytes.fromhex(curr_shift)  # convert to bytes object
#     print('Hex of Current offset: ', hex_bytes)
#     current_dec = int.from_bytes(hex_bytes, byteorder='big')  # convert to integer
#     current_bin = bin(current_dec)[2:].zfill(32) # Convert to binary 32 bit number

#     # check for the negitive binary number

#     if int(current_bin[0]) == 1:
#         current_bin= twos_complement(current_dec)
#         #print("Current dec after 2s compliment: ", current_bin)
#         # print("Current negitive shift Binary no: ", current_bin )
#         current_dec=int(current_bin)
#         #print("Current shift in Decimal :", current_dec)
#         curr_in_Hz = current_dec*3.725*1E-9
#         print("Current shift in Hz:", curr_in_Hz)
#     elif int(current_bin[0]) == 0:
#         current_dec=int(current_bin,2)
#         #print("Current shift in Decimal :", current_dec)
#         curr_in_Hz = current_dec*3.725*1E-9
#         print("Current shift in Hz:", curr_in_Hz)
#     else:
#         print("Current Dec Can not be processed/EXECUTED")
#         exit()
...
# # Method to send command to Rb.  This will act ONLY when it is triggered "signal" is set 

# def send_cmd_Rb(PID_out, lock_flag):

#     signal.wait()
    
#     if lock_flag ==0: # PID is not in lock mode 
#         New_shift = PID_out
#         #print("Correction to be applied in Hz: ", PID_out)
#     else:
#         curr_value = read_current_Rb()  # Get the current frequency offset value from the Rb
#         '''if (abs(curr_value) > 0.006): # this condition is to avoid +0.062482 Hz sudden jump
#             if (first_time == 0)& (Fst ==0 ):
#                 New_shift = -0.00335
#                 Fst =1
#             elif first_time == 0:
#                 New_shift = prev_correction
#         else:'''
#         New_shift = PID_out + curr_value
        
#     prev_correction = New_shift
#     if (abs(New_shift) > 0.9) & (steer_action == 1):
#         print("There is a Jump in Reading the Rb offset value ")
#         signal.clear()
    
#     elif 0 < abs(New_shift) <= 1:
#         print("Correction can be applied")
#         #print("New shift will be : ", PID_out)
#         if New_shift < 0:
#             #req_shift = '0x{0:08x}'.format(round((abs(new_shift_a)/3.725)*1e9))
#             New_shift_neg = round((abs(New_shift)/3.725)*1e9)
#             Nbin_New_shift = bin(New_shift_neg)[2:].zfill(32)
#             inverse_bin = ''.join(['1' if i == '0' else '0'for i in Nbin_New_shift])
#             # Add one to the binary inverted binary no
#             New_shift_Nout_bin2s = add_one(inverse_bin)
#             #print("binary twos compl zzzzzz: ",PID_Nout_bin2s)
#             New_shift_Nout_dec = int(New_shift_Nout_bin2s,2)
#             New_shift_out_hex = struct.pack('>L', New_shift_Nout_dec).hex()
#             #print("New Negitive shift in hex: ",New_shift_out_hex)
            
#         elif New_shift >=0:
#             New_shift_Pout = round((abs(New_shift)/3.725)*1e9)
#             New_shift_Pout_bin = bin(New_shift_Pout)[2:].zfill(32)
#             New_shift_Pout_dec = int(New_shift_Pout_bin,2)
#             New_shift_out_hex = struct.pack('>L', New_shift_Pout_dec).hex()
#             #print(" NEW Positive Shift in hex: ",New_shift_out_hex)
            

#         #print("Check sum:", compute_checksum(PID_out_hex))
#         #print("Writing command to RAM" )

#         cmnd = "2e 09 00 27"+" "+ New_shift_out_hex[0:2]+" "+ New_shift_out_hex[2:4]+" "+ New_shift_out_hex[4:6]+" "+ New_shift_out_hex[6:8]+" "+ New_shift_out_hex[8:10]+compute_checksum(New_shift_out_hex).replace("0x","") #format for python serial communications
#         #print("Command to be send is : ", cmnd)
#         #cmnd = struct.pack('>L', cmnd_temp).hex()
...
#         #Send_cmd_to_Rb(cmnd)
#     elif lock_flag == 0 :
#         # It means PID is not locked yet and the correction that should be applied is more than 1 Hz
#         if  abs(New_shift) > 1:
#             PID_out = PID_out/2            
#         send_cmd_Rb(PID_out, lock_flag)
                        
#     else:
#         print("The applied correction is out of Lock range: operating in lock mode ")
