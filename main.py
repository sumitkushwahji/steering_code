...
from timing_mode import Timing_mode
...
def main():
    ...
    curr_in_hz, current_value, current_value_bytes, check_sum_match = rb_device.read_current()
    ...
    latest_file, mod_time, readable_time = latest_file_in_directory(directory, file_prefix)
    if latest_file:
        print(f"Latest file: {latest_file}")
        print(f"Modification time: {mod_time} (readable: {readable_time})")
        
        combined_data, unique_mjd, unique_sv, unique_frc = read_file_data(os.path.join(directory, latest_file))
        print("Combined Data:")
        ...
        # Process CV Data
        result, unique_mjd = process_CV(df1, df2, unique_MJD_times, unique_SATs, Freq_used)
        ...
        apply_CV_corrections(CV_session, Present_error, Prev_error, time_bw_errors, correction_delay, rb_device.send_command)
